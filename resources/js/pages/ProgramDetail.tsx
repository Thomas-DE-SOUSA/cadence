...
const SESSION_STYLES: Record<string, { label: string; dot: string; text: string }> = {
    EASY: { label: 'Footing', dot: 'bg-emerald-500', text: 'text-emerald-600' },
    LONG: { label: 'Sortie longue', dot: 'bg-sky-500', text: 'text-sky-600' },
    THRESHOLD: { label: 'Seuil', dot: 'bg-orange-500', text: 'text-orange-600' },
    INTERVALS: { label: 'Fractionné', dot: 'bg-red-500', text: 'text-red-600' },
    RECOVERY: { label: 'Récupération', dot: 'bg-teal-500', text: 'text-teal-600' },
    RACE_PACE: { label: 'Allure course', dot: 'bg-fuchsia-500', text: 'text-fuchsia-600' },
    RACE: { label: 'Course', dot: 'bg-brand-500', text: 'text-brand-600' },
    CROSS: { label: 'Cross-training', dot: 'bg-indigo-500', text: 'text-indigo-600' },
    REST: { label: 'Repos', dot: 'bg-neutral-400', text: 'text-neutral-400' },
};
...
export default function ProgramDetail({ program, available, cycles, roadmap, canGenerate, activeCycleId, athlete }: Props) {
    ...
    return (
        <>
            ...
            {/* Header */}
            <div className="mb-6 rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm shadow-neutral-200/60">
                <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    {aiAction && (
                        <button
                            ...
                            className={`inline-flex shrink-0 cursor-pointer items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold transition-colors ${
                                aiAction === 'generate'
                ...
                <div className="mt-4 flex flex-wrap gap-x-8 gap-y-3 border-t border-neutral-100 pt-4">
                    <Stat value={cycles.length} label="Cycles" />
                    <Stat value={`${achieved}/${program.objectives.length}`} label="Objectifs" />
                    <Stat value={program.activities.length} label="Sorties" />
                    {athlete && <Stat value={athlete.vdot.toFixed(0)} label="VDOT" />}
                </div>
            </div>
                                ...
                                {program.objectives.map((o) => (
                                    <li key={o.id}>
                                        ...
                                        <div className="h-1.5 overflow-hidden rounded bg-neutral-100">
                                            <div
                                                className={`h-full rounded ${o.achieved ? 'bg-emerald-500' : 'bg-brand-400'}`}
