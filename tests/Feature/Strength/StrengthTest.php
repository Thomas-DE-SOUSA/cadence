...
describe('Feature: Strength', function (): void {
    it('renders the agenda week with 7 days', function (): void {
        $this->get('/strength')->assertInertia(
            fn (AssertableInertia $page) => $page->component('StrengthSchedule')->has('days', 7)->has('templates')->has('weekLabel'),
        );
    });
    ...
    it('seeds a global exercise library visible in the template editor', function (): void {
        ...
        $this->get('/strength/sessions/new')->assertInertia(
            fn (AssertableInertia $page) => $page->component('StrengthTemplate')->has('catalog')->has('muscles')->has('equipments'),
        );
    });
    ...
    it('adds a custom exercise to the tenant library', function (): void {
        ...
        $this->post('/strength/exercises', ['name' => 'Tirage prise serrée', 'primaryMuscle' => 'BACK', 'equipment' => 'CABLE'])
            ->assertRedirect();
