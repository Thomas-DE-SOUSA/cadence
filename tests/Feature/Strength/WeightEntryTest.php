...
describe('Feature: weight tracking', function (): void {
    it('logs morning and evening readings and averages the week on the Poids page', function (): void {
        $this->post('/strength/weight', ['date' => '2026-08-31', 'moment' => 'MORNING', 'weightKg' => 72.0])
            ->assertRedirect('/strength/weight');
        $this->post('/strength/weight', ['date' => '2026-08-31', 'moment' => 'EVENING', 'weightKg' => 73.0]);

        expect(WeightEntryModel::query()->where('tenant_id', 'tenant-thomas')->count())->toBe(2);

        $this->get('/strength/weight')->assertInertia(
            fn (AssertableInertia $page) => $page
                ->component('StrengthWeight')
    ...
    it('overwrites the same day + moment instead of duplicating', function (): void {
        $this->post('/strength/weight', ['date' => '2026-08-31', 'moment' => 'MORNING', 'weightKg' => 72.0]);
        $this->post('/strength/weight', ['date' => '2026-08-31', 'moment' => 'MORNING', 'weightKg' => 71.5]);
    ...
    it('rejects an out-of-range weight', function (): void {
        $this->post('/strength/weight', ['moment' => 'MORNING', 'weightKg' => 3])
